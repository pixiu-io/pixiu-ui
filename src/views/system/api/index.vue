      ...
      columnsFactory: () => [
        {
          ...
          label: '资源',
          minWidth: 120,
          formatter: (row) => h('span', { style: { fontSize: '12px' } }, row.group || '-')
        },
        {
          prop: 'method',
          label: '请求方法',
          minWidth: 120,
          formatter: (row) =>
            h(ElTag, { type: getMethodTagType(row.method), size: 'small' }, () => row.method)
        },
        {
          prop: 'description',
          label: '动作',
...
<style lang="scss" scoped>
  ...
  .api-page :deep(.art-table .el-table) {
    flex: 1 1 0;
    min-height: 0;
