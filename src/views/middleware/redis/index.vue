...
<template>
  <div class="redis-page art-full-height">
    <ElAlert
      ...
      show-icon
      class="quota-alert"
      description="管理 Redis 数据源，支持连接探测、实例概览与 Key 浏览维护（新增/删除/修改 TTL）。请先添加 Redis 数据来源，类型为中间件，再进行管理。"
            ...
            <span class="redis-datasource-wrap">
              ...
              <div v-if="selectedDatasource" class="redis-health-info">
                <span class="redis-health-info-item">
                  连接状态：<span class="redis-health-status-value" :class="pingBadgeClass">{{
                    pingStatusText
                  }}</span>
                </span>
                <span v-if="pingData?.connected" class="redis-health-info-item"
                  >延迟：{{ pingData.latencyMs }}ms</span
                >
                <span v-if="infoData?.redisVersion" class="redis-health-info-item"
                  >版本：{{ infoData.redisVersion }}</span
                >
                <span v-if="infoData?.redisMode" class="redis-health-info-item"
                  >模式：{{ infoData.redisMode }}</span
                >
                <span v-if="infoData?.usedMemoryHuman" class="redis-health-info-item"
                  >内存：{{ infoData.usedMemoryHuman }}</span
                >
                <span v-if="infoData" class="redis-health-info-item"
                  >客户端连接数：{{ infoData.connectedClients }}</span
                >
                <span v-if="infoData" class="redis-health-info-item"
                  >Key 总数（DB {{ currentDb }}）：{{ infoData.totalKeys }}</span
                >
                <span v-if="isClusterMode" class="redis-cluster-tip"
                  >cluster 模式：INFO/DBSIZE 为单节点视角，数据量统计仅代表单个节点</span
                >
              </div>
            </span>
          </div>
                    ...
                    <template #default="{ row }">
                      <div class="redis-ops">
                        <ElLink
                          ...
                          @click.stop="handleViewKey(row as RedisKeyItem)"
                        >
                          查看
                        </ElLink>
                        <ElLink
      ...
      <div v-loading="detailLoading" class="redis-detail">
        <template v-if="keyDetail">
          <ElDescriptions :column="1" border size="small" class="redis-detail__meta">
            ...
            <ElDescriptionsItem label="内存占用">{{
              formatSize(keyDetail.sizeBytes)
            }}</ElDescriptionsItem>
          </ElDescriptions>
          ...
          <ElInput
            ...
            :model-value="
              typeof keyDetail.value === 'string'
                ? keyDetail.value
                : JSON.stringify(keyDetail.value)
            "
            type="textarea"
            :rows="6"
            readonly
    ...
    >
      <ElForm label-width="90px" label-position="left">
        ...
        <ElFormItem v-if="createForm.ttlMode === 'expire'" label="TTL">
          <ElInputNumber v-model="createForm.ttl" :min="1" :max="31536000" style="width: 120px" />
    ...
    <ElDialog v-model="ttlVisible" title="修改 TTL" width="440px" destroy-on-close>
      <ElForm label-width="90px">
        ...
        <ElFormItem v-if="ttlForm.mode === 'expire'" label="TTL">
          <ElInputNumber v-model="ttlForm.seconds" :min="1" :max="31536000" style="width: 120px" />
          <span class="redis-ttl-unit">秒</span>
        </ElFormItem>
      </ElForm>
      <template #footer>
...
<script setup lang="ts">
  ...
  async function submitCreateKey() {
    ...
    if (
      createForm.value.ttlMode === 'expire' &&
      (!createForm.value.ttl || createForm.value.ttl < 1)
    ) {
  ...
  async function openEditDialog(row: RedisKeyItem) {
    ...
    try {
      ...
      editForm.value.value =
        typeof detail.value === 'string' ? detail.value : String(detail.value ?? '')
      if (detail.truncated) {
