  ...
  <div class="es-page art-full-height">
          ...
          <div class="es-rule-left">
            ...
            <span class="es-datasource-wrap">
              ...
              <div v-if="selectedDatasource" class="es-health-info">
                <span class="es-health-info-item"
                  >健康状态：<span class="es-health-status-value" :class="healthBadgeClass">{{
                    healthStatusText
                  }}</span></span
                >
                <span v-if="healthData?.cluster_name" class="es-health-info-item"
                  >集群名称：{{ healthData.cluster_name }}</span
                >
                <span v-if="healthData?.version_number" class="es-health-info-item"
                  >版本：{{ healthData.version_number }}</span
                >
                <span class="es-health-info-item"
                  >在线节点数：{{ healthData?.number_of_nodes ?? '-' }}/{{
                    healthData?.number_of_data_nodes ?? '-'
                  }}</span
                >
                <span class="es-health-info-item"
                  >总分片数：{{ healthData?.active_shards ?? '-' }}</span
                >
                <span class="es-health-info-item"
                  >未分配分片数：{{ healthData?.unassigned_shards ?? '-' }}</span
                >
              </div>
            </span>
          </div>
    ...
    <div v-if="activeTab === 'index'" class="es-index-toolbar">
      <div class="es-index-toolbar__left">
        ...
        <ElButton v-ripple :disabled="!selectedIndices.length" @click="handleBatchDelete"
          >批量删除</ElButton
        >
      </div>
          ...
          <div class="es-index-table-wrap">
            <div class="es-index-table-scroll">
              ...
              >
                ...
                />
                <ElTableColumn
                  v-if="isIndexColVisible('health')"
                  prop="health"
                  label="健康状态"
                  min-width="100"
                >
                  <template #default="{ row }">
                    <span :style="{ color: healthColorMap[row.health] || 'inherit' }">{{
                      healthLabelMap[row.health] || row.health
                    }}</span>
                  </template>
                </ElTableColumn>
                <ElTableColumn
                  v-if="isIndexColVisible('status')"
                  prop="status"
                  label="状态"
                  width="70"
                />
                <ElTableColumn
                  v-if="isIndexColVisible('pri')"
                  prop="pri"
                  label="主分片"
                  min-width="90"
                  align="right"
                ...
                <ElTableColumn label="读写状态" width="80" align="center">
                  <template #default="{ row }">
                    <span
                      :style="{ fontSize: '12px', color: row.read_only ? '#dc2626' : '#16a34a' }"
                    >
                      {{ row.read_only ? '只读' : '读写' }}
                    </span>
                  </template>
          ...
          beforeClose: async (action, instance, done) => {
            ...
            for (const row of selectedIndices.value) {
              try {
                if (ds.external) {
                  const headers = getExternalProxyHeaders(ds)
                  await pixiuAxios.delete('/pixiu/external', {
                    params: { url: `${baseUrl}/${row.index}` },
                    headers
                  })
                } else {
    ...
    try {
      ...
      if (ds.external) {
        ...
        const res = await pixiuAxios.get(
          `/pixiu/external/_cat/indices/${encodeURIComponent(indexPattern)}`,
          {
            params,
            headers
          }
    ...
    try {
      ...
      if (ds.external) {
        ...
        await pixiuAxios.put(
          `/pixiu/external/${encodeURIComponent(row.index)}/_settings`,
          payload,
          {
            params: { url: baseUrl },
            headers
...
<style lang="scss" scoped>
  ...
  .es-health-badge.is-loading {
    ...
    .es-health-dot {
      ...
      animation: es-pulse 1.2s infinite;
    }
  }
