          ...
          <div class="rq-rule-left">
            ...
            <span class="rq-datasource-wrap">
              ...
              >
                ...
                <ElOption v-for="ds in filteredDsList" :key="ds.id" :label="ds.name" :value="ds.id">
      ...
      <div class="rq-query-body">
        <div class="rq-query-toolbar">
          <div class="rq-query-toolbar-left">
            <span
              :class="['rq-query-tab', { 'is-active': activeQueryTab === 'promql' }]"
              @click="activeQueryTab = 'promql'"
            >
              PromQL语句
            </span>
            <span
              :class="['rq-query-tab', { 'is-active': activeQueryTab === 'favorite' }]"
              @click="activeQueryTab = 'favorite'"
            >
              收藏夹
            </span>
            <span
            ...
            <span
              :class="['rq-query-tab', { 'is-active': activeQueryTab === 'template' }]"
              @click="activeQueryTab = 'template'"
            >
              语句模板
            </span>
          </div>
              ...
              >
                <template #default="{ item }">
                  <div class="rq-suggestion-item">
                    <span class="rq-suggestion-kind" :class="`is-${item.kind}`" aria-hidden="true">
                      <ArtSvgIcon
                    ...
                    <span class="rq-suggestion-text">
                      <template v-for="(part, idx) in getSuggestionParts(item.value)" :key="idx">
                        <span v-if="part.highlight" class="rq-suggestion-text-match">{{
                          part.text
                        }}</span>
...
<script setup lang="ts">
  ...
  // ---- 查询 ----
  const { history, showHistory, addRecord, removeRecord, clearAll, closeHistory } =
    useQueryHistory()
  ...
  const { getAxisLineStyle, getSplitLineStyle, getAxisLabelStyle, getTooltipStyle } = useChart()
  ...
  function resolveGraphLineOpacity(_seriesCount: number): number {
    return 0.96
  }
